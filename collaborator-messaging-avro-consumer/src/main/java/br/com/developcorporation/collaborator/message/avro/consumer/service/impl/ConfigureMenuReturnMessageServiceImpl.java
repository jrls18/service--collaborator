...
@Service
public class ConfigureMenuReturnMessageServiceImpl implements ConfigureMenuReturnMessageService<ConfigureMenuReturn> {
    ...
    @KafkaListener(topics = "${kafka.topic.consumer.configure.menu.return}", groupId = "${kafka.topic.consumer.configure.menu.return.id}")
    @Override
    public void onReceive(ConsumerRecord<String, ConfigureMenuReturn> record) {
        ...
        collaboratorService.updateStatusCollaboratorAsync(
                messageAsync.getObj(),
                record.value().getUser().getIsError(),
                true);
